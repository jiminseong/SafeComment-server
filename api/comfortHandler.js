...
const comfortHandler = async (req, res) => {
  if (req.method !== "POST") {
...
악플이 아니면 "false"만을 반환해줘. 악플이라면 그저 위로글만 작성해주면 돼.

무엇보다, 마크다운이나 , 글자 굵게 하기 없이 텍스트만으로 구성되도록 해주고, 따뜻한 문체로 작성해줘.
그리고 사용자는 제작자일 수도 있고, 시청자일 수도 있어.
그래서 제작자라면~, 시청자라면~ 이런 식으로 두 가지로 나눠서 표현해줘.
`;
